...
public class Client
{
     ...
     *
     * @param game
     *     Instance of {@link MessengerJava}.
     * @param protocol
     *     Protocol for connection.
     * @param host
     *     Host of server for connection.
     * @param port
     *     Port of server for connection.
     * @param hints
     *     {@link SocketHints} for connection.
     * @since 1.0
     */
            ...
            while (true) {
                ...
                try {
                    ...
                    if (receivedBytes != availableBytes) {
                        ...
                        this.game.logger.warning(String.format(
                            Locale.ENGLISH,
                            "Incorrect read size: %d / %d", receivedBytes, availableBytes
                        ));
                    }
                } catch (IOException e) {
                    this.game.logger.severe(e.getMessage());
                ...
                if (this.currentScreen != null) {
                    ...
                    while (true) {
                        if (this.receiveBytes.isEmpty()) {
                            break;
                        }

                        lastBytes = this.receiveBytes.removeFirst();
                        @SuppressWarnings("ObjectAllocationInLoop")
                        ByteBuffer packet = ByteBuffer.wrap(lastBytes);
    ...
    public boolean reconnect() {
        try {
            this.socket = Gdx.net.newClientSocket(
                this.protocol,
                this.host,
                this.port,
                this.socketParams
